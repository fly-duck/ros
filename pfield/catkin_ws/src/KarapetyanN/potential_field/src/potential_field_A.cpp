 ...
 poseSubs.push_back(nh.subscribe(\
  ...
  void laserCallback(const sensor_msgs::LaserScan::ConstPtr& msg) {
      ...
      for (int i = 0; i < numRobots; i++) {
          cumulative_x = 0.0;
          cumulative_y = 0.0;
          double repulsive_x = 0.0; // x coordinate of repulsive force in stage coordinate system; (cumulative repulsive for all rays)
          double repulsive_y = 0.0; // y coordinate of repulsive force in stage coordinate system;
          ...
          cumulative_x = currRay_x_st; //+ pose[i].x;
          cumulative_y = currRay_y_st; //+ pose[i].y;
          // DEBUG-------------------------------
          std::cout  << "-----> ONLY REPULSIVE from LASERCALLBACK ----------------------------------\n";
          std::cout  << "---------------------------------------\n";
          std::cout  << "-- Repulsive -- "<< cumulative_x << " " << cumulative_y <<" ---------------";
          std::cout  << "---------------------------------------\n";
          std::cout  << "-----ONLY REPULSIVE from LASERCALLBACK <----------------------------------\n";
          // DEBUG-------------------------------
      }
...
//    double attractive_theta = tf::getYaw(msg->pose.pose.orientation);
              ...
              std::cout  << "-----> ONLY REPULSIVE----------------------------------\n";
              std::cout  << "-- Current Pose -- "<< pose[i].x << " " << pose[i].y <<" ---------------";
              std::cout  << "---------------------------------------\n";
              std::cout  << "-- Cumulative -- "<< cumulative_x << " " << cumulative_y <<" ---------------";
              std::cout  << "-----ONLY REPULSIVE<----------------------------------\n";
              // DEBUG-------------------------------!
              ...
              if(d > 0) {
                  if(d < D_SAFE) {
                      ...
                      if(fabs(cumulative_theta - pose[i].heading) > EPS) {
                          //fsm = FSM_ROTATE;
                  ...
                  /*if(prevPose.x == pose[i].x && prevPose.y == pose[i].y){
					...
					rotateDuration = ros::Duration(double(1 + (rand()%7)) ); //to rotate in [ pi/4 , 3*Pi/2] range
                      while(moveStartTime - ros::Time::now() < moveDurationi && d > D_SAFE) {
                      move(linear_v,0);
                      }
                  }*/
              }
  ...
  const static double D_SAFE = 0.5; // in meters
  const static double G = 1.0; // gamma
  const static double K = 1.0; // scaling constant
  const static double A = 1.0; //alpha
  const static double B = 1.0; //betta in meters
...
protected:
  ...
  double cumulative_x ; // x coordinate of repulsive force in stage coordinate system; (cumulative repulsive for all rays)
  double cumulative_y; // y coordinate of repulsive force in stage coordinate system;
